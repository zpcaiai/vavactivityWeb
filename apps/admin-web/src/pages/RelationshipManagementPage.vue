...
<style scoped>
.relationship-admin{display:grid;gap:1rem}header{display:flex;justify-content:space-between;gap:2rem;align-items:end}header>p{max-width:620px}.eyebrow{letter-spacing:.12em;color:var(--vav-color-accent)}nav{display:flex;gap:.55rem;flex-wrap:wrap}nav a{padding:.5rem .8rem;border-radius:999px;background:var(--vav-color-surface-soft);color:var(--vav-color-text);text-decoration:none}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:1rem}.metrics article,.panel{padding:1rem;border:1px solid var(--vav-color-border);border-radius:12px;background:white}.metrics strong{display:block;font-size:1.8rem;margin-top:.5rem}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:.75rem;border-bottom:1px solid var(--vav-color-surface-sunken)}pre{overflow:auto;padding:1rem;background:var(--vav-color-surface-soft)}.safety{display:grid;gap:.7rem;margin-top:1rem;padding:1rem;background:var(--vav-color-surface-warning)}.safety input{padding:.65rem;margin-right:.5rem}.safety button{margin:.3rem;padding:.6rem .9rem}.danger{background:var(--vav-color-danger);color:white}.alert{padding:.8rem}.error{background:var(--vav-color-surface-danger)}.notice{background:var(--vav-color-surface-success)}
</style>
